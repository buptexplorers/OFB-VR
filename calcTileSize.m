...
function [tileSize] = calcTileSize(set, vid,sec,sr,er,sc,ec,tileH,tileW,qp) %TODO 调整其他程序
...
hP = tileH * (er-sr+1);
wP = tileW * (ec-sc+1);
tilePath = sprintf('tiles/%d/%03d/%03d/%04d_%04d_%04d_%04d_%04d.mp4',set,vid,sec,wP,hP,scP,srP,qp);
%check if it exists
if ~exist(tilePath,'file')
    mkdir(sprintf('tiles/%d/%03d/%03d',set,vid,sec));
