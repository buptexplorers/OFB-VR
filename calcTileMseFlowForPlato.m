function [viewedTiles,PMSE] = ...
    calcTileMseFlowForPlato(set, vid,sec,frame,tiling,tileW,tileH,transRealOrPred,calcMSERealOrPred,only42and22)
...
for user=1:usernum%%%DEBUG
    ...
    DepthFlo = sum(abs(tileFlo).^2,3).^(1/2);%逐位求模
    ...
    for i = 1:frameGap
        ...
        %计入总运动向量中
        speedFlo = speedFlo + tileFlo/1440*2*pi;%换为度数角速度
    end
