function [PSPNR, sumSize, sumViewedTilesArea, viewportQPsizePerGrid] = PanoFlow(set,vid,sec,tileW, tileH, sizeLmt,nExtra) % 与baseline.m对称
% sizeLmt   nUser * nQP
nUser = 10;
...
if ~exist(sprintf('MSE/%d/%03d/%03d_Pano_Flow_pred.mat',set,vid,sec))
    ...
    load(sprintf('MSE/%d/%03d/%03d_Pano_Flow_pred.mat',set,vid,sec));
end

if ~exist(sprintf('MSE/%d/%03d/%03d_Pano_Flow_real.mat',set,vid,sec))
    ...
    load(sprintf('MSE/%d/%03d/%03d_Pano_Flow_real.mat',set,vid,sec));
end
...
%     imshow(pixels,[]);
    ...
    for qp=22:42+nExtra
        ...
        try
            ...
            disp(['PanoFlow ',num2str(sum(min(temp))),' Limit ',num2str(newSizeLmt)]);
        end
    end
end
end