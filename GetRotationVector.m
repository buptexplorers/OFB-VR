function [result]=GetRotationVector(videoId,userId,framegap)%一次性获取帧间隔为framegap的对应videoid userid的旋转速度
...
for j=videoId:videoId %代表视频号
    ...
    for num=userId:userId  %代表用户
        ...
        for i=1:x
            q2=data_list(i,:);
            tempTimexyz=[2*q2(1)*q2(3)+2*q2(2)*q2(4),2*q2(2)*q2(3)-2*q2(1)*q2(4),1-2*q2(1)*q2(1)-2*q2(2)*q2(2)];%四元数换算为xyz
        ...
        for i=1:framegap:floor(time_list(x,1)/0.033) %以framegap为间隔
            ...
            Rotationspeed=abs(CalRotationVector(dataToBeHandled)*1.0/(framegap*1.0/30));
