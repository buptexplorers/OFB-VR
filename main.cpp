...
using namespace std;
vector<string> rs;
const vector<int> mGroup{721,991,1021,1171,1411,1441,1531,1591,1681,1711};
...
void read_write()
{
    ...
    for (int set=1; set<=1; ++set)//org set<=2
    for (int video = 0; video <= 0; ++video) {//org video<=8
        //if (video == 5) continue;
        for(int i = 0; i <= 10; ++i) {
            flag = 0;
            for (int user = 1; user <= 48; ++user) {
                ...
                strcat(filename1,filename_body);
                memset(filename_body, 0, sizeof(filename_body));
                sprintf(filename_body,"%d",mGroup[i]);
                strcat(filename1,filename_body);
                char filename_tail1[50] = "_Value_PMSE.txt";//org _VRJND_Value_PMSE.txt
                strcat(filename1,filename_tail1);
                if((fp=fopen(filename1, "r")) == NULL)
                {
                    cout<<"open "<<filename1<<" fail..."<<endl;
                    continue;
                }
                flag = 1;
                ...
                char filename2[100] = "viewpoint/";//org viewport
                memset(filename_body, 0, sizeof(filename_body));
                sprintf(filename_body,"%d",set);
