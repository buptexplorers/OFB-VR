...
int QP[SumUser][SIZEX + 1][SIZEY + 1] , QP0[SumUser][SIZEX + 1][SIZEY + 1] , realcut;
...
void read_write()
{
    ...
    fq = NULL;//output
    avgarea = avgbias = 0;
    char filename_body[20];
    for (int set=1; set<=1; ++set)//change the number
    //for (int video = 2; video <= 2; ++video) {//change the number
    for (int video = 0; video <= 2; ++video) {//change the number
        //if (video == 2 || video == 4 || video == 5) continue; 
        //for (int i = 121; i <= 1771; i += 150) {//change the number
        for(int i = 301; i <= 2071; i +=30){
            flag = 0;
            for (int user = 1; user <= SumUser; ++user) {//SumUser
                //char filename1[50] = "set5/";//../../ratio/";
                char filename1[100] = "E:/MATLAB/DSPexp/PanoProject-master/SIGCOMM Source Code/ratio/";
                memset(filename_body, 0, sizeof(filename_body));
                sprintf_s(filename_body,"%d",set);
                strcat(filename1,filename_body);
                memset(filename_body, 0, sizeof(filename_body));
                sprintf_s(filename_body,"/");
                strcat(filename1,filename_body);
                memset(filename_body, 0, sizeof(filename_body));
                sprintf_s(filename_body,"%d",video);
                strcat(filename1,filename_body);
                memset(filename_body, 0, sizeof(filename_body));
                sprintf_s(filename_body,"/");
                strcat(filename1,filename_body);
                memset(filename_body, 0, sizeof(filename_body));
                sprintf_s(filename_body,"%d",user);
                strcat(filename1,filename_body);
                memset(filename_body, 0, sizeof(filename_body));
                sprintf_s(filename_body,"/");
                strcat(filename1,filename_body);
                memset(filename_body, 0, sizeof(filename_body));
                sprintf_s(filename_body,"%d",i);
                strcat(filename1,filename_body);
                char filename_tail1[50] = "_Value_SMSE.txt";//"_VRJND_Value_PMSE.txt";
                //char filename_tail1[50] = ".txt";
                strcat(filename1,filename_tail1);
                if((fp=fopen(filename1, "r")) == NULL)
                {
                    cout<< filename1<<"open filename fail..."<<endl;
                    continue;
                }
                //else cout << filename1 << "OK!!!!!!!!!" << endl;
                ...
                strcat(filename2,filename_body);
                memset(filename_body, 0, sizeof(filename_body));
                sprintf_s(filename_body,"/realviewport_");
                strcat(filename2,filename_body);
                memset(filename_body, 0, sizeof(filename_body));
                sprintf_s(filename_body,"%d",i);
                strcat(filename2,filename_body);
                ...
                {
                    cout << filename2 <<"open filename fail..."<<endl;
                    continue;
                }
                flag = 1;
                for (int j = 0; j < SIZEX; ++j)
			...
			char dir[100] = "E:/MATLAB/DSPexp/PanoProject-master/SIGCOMM Source Code/tilingDP/Project1/tiling1004/";
				//"../../tiling1004/";
            char dir_name[100];
            memset(dir_name, 0, sizeof(dir_name));
            sprintf_s(dir_name,"%d",set);
            strcat(dir,dir_name);
            memset(dir_name, 0, sizeof(dir_name));
            sprintf_s(dir_name,"/");
            strcat(dir,dir_name);
            memset(dir_name, 0, sizeof(dir_name));
            sprintf_s(dir_name,"%d",video);
            strcat(dir,dir_name);
            memset(dir_name, 0, sizeof(dir_name));
            sprintf_s(dir_name,"/");
            strcat(dir,dir_name);
            memset(dir_name, 0, sizeof(dir_name));
            sprintf_s(dir_name,"%d",i);
            strcat(dir,dir_name);
            
            int status = _mkdir(dir);//mkdir(dir, S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
            printf("%d\n" , i);
            
            for (int k = 1; k <= 1; ++k) { //DEBUG SumUser???
                char outputfile[100] = "E:/MATLAB/DSPexp/PanoProject-master/SIGCOMM Source Code/tilingDP/Project1/tiling1004/";
