...
% transRealOrPred��ʾ������ʲô�ӵ㣬Ψһ��������ȷ������Щ�飬calcMSERealOrPred��ʾ��MSE��ʲô
% only42and22Ϊ0��ֻ����QP=22��42����
if only42and22==0
    QPrange = 22:42;
else
    QPrange = [22,42];
...
for user=1:usernum%%%DEBUG
    ...
    for j=transCenter(1)-314:transCenter(1)+315
        for k=transCenter(2)-419:transCenter(2)+420
            x=[j,k];
            d=sqrt((x(1)-transCenter(1))^2+(x(2)-transCenter(2))^2);%���������ĵľ���
            N=840;% �ӿڿ��
            v=0.43; %Բ����
            e=(atan(d/(N*v))/pi)*180;%Բ�η��� �뾶N*v/2
    ...
    DepthPath = strcat(DepthPath,int2str(floAbs-1),'.flo');
    ...
    DepthFlo = sum(abs(tileFlo).^2,3).^(1/2);%��λ��ģ
    DepthFlo = DepthFlo/max(max(DepthFlo));%��һ��
    ...
    DepthJND = 1+3*abs(DepthJND - referenceD); %��DepthJND������1~4��Χ�ڣ���Pano�Ա�
    ...
    if strcmp(calcMSERealOrPred,'real')==1 % ����real
    ...
    for i = 1:frameGap
        ...
        speedFloPath = strcat(speedFloPath,int2str(floAbs-1),'.flo');
        load([speedFloPath,'.mat']);
        %�������˶�������
        speedFlo = speedFlo + tileFlo/1440*2*pi;%��Ϊ�������ٶ�
    end
    ...
    for i=1:size(tiling,1)
        ...
        if sum(vtcIntscCalcMSE(:))*sum(hrzIntscCalcMSE(:))==0 % �����calcMSEviewportû�н��������ܴ�����������
