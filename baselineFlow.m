...
% output: PMSE  nQP * 1
%         Size  nQP * 1
function [PSPNR,sumSize,sumViewedTilesArea,viewportQPsizePerGrid] = baselineFlow(set,vid, sec, nGridR, nGridC)
nUser = 10;
...
%DEBUG
if ~exist(sprintf('MSE/%d/%03d/%03d_BSLF.mat',set,vid,sec))
    ...
    save(sprintf('MSE/%d/%03d/%03d_BSLF.mat',set,vid,sec),'meanMSE','viewedTiles');
else
    load(sprintf('MSE/%d/%03d/%03d_BSLF.mat',set,vid,sec));
end
